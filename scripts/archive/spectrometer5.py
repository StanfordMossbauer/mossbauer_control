...
class MossbauerSpectrometer:
    ...
    def sweep(self, frequency):
        ...
        try : 
            self.client.connect(self.hostname,self.port,self.username,self.password)
            stdin,stdout,stderr=self.client.exec_command("python3 rp_start.py")
            self.client.close()
        ...
        try : 
            self.client.connect(self.hostname,self.port,self.username,self.password)
            stdin,stdout,stderr=self.client.exec_command("python3 rp_read.py")
            self.client.close()
            #print(stdout.read().decode())
        except :
            pass
...
if __name__ == "__main__":
    ...
    scan.scanTime = 20
    scan.dutycycle = 0.8  #this is not a setting, it must be controlled on the redpitaya. here just for knowledge
